---
title: 资料分析-增长量（六）
author: Jun
tags:
  - study
categories:
  - 考公
  - 资料分析
cover: https://s21.ax1x.com/2025/08/16/pV06dYV.png
date: 2025-08-16 23:22:59
---
# 

<center>
    <h1>
        增长量
    </h1>
</center>



![pV0yzZR.png](https://s21.ax1x.com/2025/08/16/pV0yzZR.png)

## 1.公式：

> $$
> 增长量 = 现期量 - 基期量 = 基期量 * 增长率 =\frac{现期}{1+r}*r
> $$

当|r| < 5%，增长量 = 现期量 * r

## 2.年均增长量

### 2.1公式：

识别：年均 + 增长 + 单位（每一年的增长量相同）

$$
公式：年均增长量 =\frac{\text{现期 - 基期}}{\text{n}} n为年份差
$$


### 2.2年均增长类问题基期的选择

一般情况（除江苏外）：2012年 - 2018年：年份差为6 

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;基期是2012年，现期是2018年

五年规划（全国统一）： “十三五”期间：年份差为5（基期往前推一年） 

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;基期是2015年，现期是2020年

江苏省考：2012年 - 2018年：年份差为7（基期往前推一年） 

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;基期是2011年，现期是2018年

## 3.给现期量和增长率，求增长量

### 3.1公式

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;第一步：
$$
增长率百化分，|r|=\frac{1}{n}(n是分母)
$$
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;第二步：
$$
增长量=\frac{现期量}{n+1}(r > 0)
$$

$$
减少量=\frac{现期量}{n-1}(r < 0)
$$



例 1：2016 年总收入为 120 万元，同比增长 33.3%，求 2016 与 2015 相比总收入增长了多少万元？

$$ 常规做法为，\frac{现期量}{1+r}*r=\frac{120}{1+33.3\%} * 33.3\%太复杂了$$


应使用：增长率 33.3%，百化分后为 1/3，分母为 3，即 n 为 3

$$  当 n 为正时，增长量=\frac{现期量}{n+1}=\frac{120}{3+1} = 30万元$$


例 2：2016 年总收入为 120 万元，同比下降 33.3%，求 2016 与 2015 相比总收入增长了多少万元？

增长率为 -33.3%，百化分后为 1/3，分母为 3，即 n 为 3

$$ 当 n 为负时，增长量=\frac{现期量}{n-1}=\frac{120}{3-1}= 60万元$$

### 3.2误差分析

**利用现期量/n+-1算出的结果若卡在了选项AB之间该如何解决？**

**如果增长率看大了，算出的结果会偏大，实际值应该略小于算出的结果**

**如果增长率看小了，算出的结果会偏小，实际值应该略大于算出的结果**

例题：2020年j省新增发放创业贷款192.7亿元，同比增长24.4%,...

2020年j省新增发放创业贷款比上年增长了多少亿元？

A.37.8 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; B.39.6 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;C.41.2 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;D.43.5

*解析：24.4%看成25% 约等于1/4，192/4+1约等于38多点，选项差距小，这个结果卡在了AB之间，选哪个*

*因为把24.4%看成25%，是把增长率往大了看，那么结果就偏大了，所以实际值应该小于算出的结果38，四个选项只有A小于38.所以选A*

### 3.3百化分方法




$$
(1)50\%=\frac{1}{2} \quad 33\%=\frac{1}{3} \quad 25\%=\frac{1}{4} \quad 20\%=\frac{1}{5} \quad 10\%=\frac{1}{10}
$$

（2）“8-13”整数部分 + 分母部分 等于 20

$$  12.5\%=\frac{1}{8} \quad 11.1\%=\frac{1}{9} \quad 9.1\%=\frac{1}{11} \quad 8.3\%=\frac{1}{12} \quad 7.7\%=\frac{1}{13}$$


（3）记住（16，6）和（14，7）互换的两对

$$  16.7\%=\frac{1}{6} \quad 6.25\%=\frac{1}{16} \quad 14.3\%=\frac{1}{7} \quad 7.1\%=\frac{1}{14}$$


（4）记住（17，18，19）；5.963

$$  5.9\%=\frac{1}{17} \quad 5.6\%=\frac{1}{18} \quad 5.3\%=\frac{1}{19}$$

（5）记住(6.7,15.1)

$$6.7\%=\frac{1}{15}  \quad 15.1\%=\frac{1}{6.7}$$


（6）记住：

$$  18.5\%=\frac{1}{5.5} \quad 15.4\%=\frac{1}{6.5} \quad 10.5\%=\frac{1}{9.5}$$

（7）记住（2737）

$$2.7\%=\frac{1}{37}  \quad 3.7\%=\frac{1}{27}$$

### 3.4增长率百化分取中法

如果遇到百分数左右难以取舍，取中即可

如15.4%约等于几分之几

15.4左边是14，正好有个14.3是1/7

15.4右边是16，正好有个16.7是1/6

那么15.4刚好位于二者之间，即可取中为1/6.5

例题：2019年1-10月，江苏民航机场旅客吞吐量4901万人次，同比增长13.4%，...

​	2019年1-10月，江苏民航机场旅客吞吐量同比增加：

A.398万人次  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;B.435万人次 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;C.579万人次 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;D.657万人次

*解析：13.4%左侧12，刚好有12.5是1/8，右侧14，刚好有14.3是1/7，13.4介于二者之间取1/7.5，带入公式4904/7.5+1，结果为5+，所以选C*

### 3.5增长率百化分倍数法

利用与背过的百分数的倍数关系，实现百化分

65%=33% * 2 = 2/3 = 1/1.5

## 4.增长量的倍数

给现期量、增长率，求增长量的倍数

（1）增长率差在 10 个百分点以上，用 增长量 = 现期量 / n+-1，再求倍数

例 1：2024 年志哥工资 200 元，同比增长 50%，2024 年郭子工资 100 元，同比增长 5%，2024 年志哥工资同比增长量是郭子的多少倍？

增长率差在 10 个百分点之上，（50% - 5% =45%），只能分别计算,

$$  50\%=\frac{1}{2} \quad 志哥\%=\frac{200}{2+1} \quad 郭子=5\%=\frac{1}{20}\frac{100}{20+1} \quad \frac{200}{3}  / \frac{100}{21}$$

（2）增长率差在 10 个百分点以内，**增长量的倍数= 两个现期之间的倍数 \* 两个增长率之间的倍数**

$\frac{\Delta A}{\Delta B} = \frac{A_0}{B_0} \times \frac{a}{b}$(不是大数比小数，有顺序的)

其中 $A_0$、$B_0$ 为现期量，$a$、$b$ 为对应增长率（已带 %）。

<br>

<br>

**2024年S省物流数据：**

- 工业品物流总额：16635.16 亿元，同比增长 0.2%
- S省社会物流总额：35357.26 亿元，同比增长 6.4%

**问：** S省工业品物流总额同比增量约为同期社会物流总额同比增量的多少？

A. 超过30% &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;B. 10%~30%  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;C. 5%~10% &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;D. 不到5%

 





**Step 1：计算现期量倍数:**&nbsp;&nbsp;$\frac{16635}{35357}=0.4705$



**Step 2：计算增长率倍数**:&nbsp;&nbsp;$\frac{0.2}{6.4}=0.03125$



**Step 3：相乘得增长量倍数:&nbsp;&nbsp;0.4705×0.03125≈0.0147=1.47%**



**结论：** 1.47% 明显小于 5%，选 **D**。

例 1：2024 年志哥工资 200 元，同比增长 10%，2024 年郭子工资 100 元，同比增长 5%，2024 年志哥工资同比增长量是郭子的多少倍？

因为 10% - 5% = 5% < 10% 所以

$$ \frac{200}{100} * \frac{10\%}{5\%}=2 * 2 =4倍$$


### 增长量速算

2016 年电信业务收入完成 11893 亿元，同比增长 5.6%，比上年回升 7.6 个百分点。电信业务总量完成 **35948** 亿元，同比**增长 54.2%**，比上年提高 25.5 个百分点

2016 年电信业务总量同比增长了约（）亿元

<center>
    A.12653&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    B.12635&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    C.7340&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    D.7304&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
</center><br>



$$ \displaystyle 增长量=\frac{\text{现期量}}{\text{1+r}}* r    =\frac{\text{35948}}{\text{1+54.2%}}* 54.2\%=\frac{\text{35948}}{\text{154.2%}}* 54.2\% $$


分母154.2%和54.2%对消 分母变成3 ，54.2%对消了，35948/3 的结果肯定不在 CD 中，排除

然后看 AB 相近，就找一个跟选项 AB 差不多的值，假设一个增长量 12648，用真的现期量减去假的增长量得到一个假设的基期：35948-12648 = 23300 假设的基期 23300 \* 54.2%=23300 \*（55%-0.8%）=11650+1165=12815-186.4=12629 选 B
